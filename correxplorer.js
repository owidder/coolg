...
//    .style("opacity", 1e-6);
    ...
    function valueToZoomFactor(value) {
        return 1 + (value / 1000 * 2);
    }
    ...
    var scale = d3.scale.linear()
        ...
        .range([0, valueToZoomFactor(parseFloat(d3.select("input#zoomrange")[0][0].value)) * 600]);

    d3.select("#zoomrange").on("change", function () {
        var zoomfactor = valueToZoomFactor(parseFloat(this.value));
        scale = d3.scale.linear()
            .domain([0, d3.min([50, d3.max([label_col.length, label_row.length, 4])])])
            .range([0, zoomfactor * 600]);
